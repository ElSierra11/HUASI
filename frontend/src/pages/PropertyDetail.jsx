...
export default function PropertyDetail() {
  ...
  const handleBooking = async (e) => {
    ...
    const confirmed = await HuasiAlert.confirm({
      title: '¿Confirmar solicitud de hospedaje?',
      text: `Solicitarás estancia del ${new Date(booking.fecha_inicio + 'T00:00:00').toLocaleDateString('es-CO')} al ${new Date(booking.fecha_fin + 'T00:00:00').toLocaleDateString('es-CO')} para ${booking.num_huespedes} ${booking.num_huespedes === 1 ? 'huésped' : 'huéspedes'}.`,
    ...
    try {
      ...
      HuasiAlert.success(
        '¡Solicitud Enviada!',
        'El anfitrión ha recibido tu solicitud de hospedaje solidario. Te notificaremos cuando la acepte.'
      );
      ...
      const msg = err.response?.data?.error || 'Error al solicitar reserva';
      setError(msg);
      HuasiAlert.error('No se pudo enviar la solicitud', msg);
    } finally {
    ...
    <div>
      <div className="detail-header">
        ...
        <div className="detail-meta" style={{ flexWrap: 'wrap', gap: '16px' }}>
          ...
          <span className="badge" style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: 'rgba(16, 185, 129, 0.12)', color: '#15803d', border: '1px solid rgba(16, 185, 129, 0.3)', fontWeight: 700, fontSize: '0.78rem' }}>
            <ShieldCheck size={14} /> Verificado y Aprobado UCC
          </span>
          {isOwner && (
        ...
        <div className="detail-info">
          ...
          <section>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 12 }}>
              <h2 style={{ margin: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
                <MapPin size={22} color="var(--ucc-green)" /> Ubicación y Cercanía UCC
              </h2>
              {(() => {
                const coords = getPropertyCoordinates(prop);
                const gmaps = getGoogleMapsUrl(coords.lat, coords.lng, `${prop.direccion || ''}, ${prop.ciudad || ''}`);
                const waze = getWazeUrl(coords.lat, coords.lng);
                return (
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    ...
                    <a
                      ...
                      style={{
                        ...
                        padding: '7px 14px',
                        borderRadius: 8,
                        boxShadow: '0 2px 6px rgba(51,204,255,0.3)',
                        textDecoration: 'none'
                      }}
                    >
            ...
            <p style={{ color: 'var(--text-muted)', marginBottom: 14, fontSize: '0.92rem' }}>
              ...
              {prop.campus_cercano && <span style={{ marginLeft: 8, color: 'var(--ucc-green)', fontWeight: 600 }}>• Sede: {prop.campus_cercano}</span>}
            </p>
          ...
          <section style={{ borderBottom: 'none', padding: 0 }}>
            <SistemaReputacion
              ...
              propiedadId={prop.id}
              userReservation={prop.ya_reservado}
              onReviewAdded={loadPropiedad}
            />
          </section>
                    ...
                    {prop.ya_reservado.estado === 'aceptada' && prop.direccion_exacta && (
                      <div style={{ marginTop: 12, padding: 12, background: 'rgba(16, 185, 129, 0.1)', borderRadius: 8, border: '1px solid rgba(16, 185, 129, 0.2)', textAlign: 'left' }}>
                        <strong style={{ color: 'var(--success)', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                          <MapPin size={16} /> Dirección exacta revelada:
                        </strong>
