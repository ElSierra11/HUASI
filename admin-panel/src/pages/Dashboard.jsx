...
export default function Dashboard() {
  ...
  const handleAprobar = async (id) => {
    if (!window.confirm('¿Estás seguro de aprobar esta solicitud de verificación?')) return;
    try {
      await api.patch(`/auth/verificacion/admin/${id}`, { estado: 'aprobado', notas: 'Verificado por el administrador' });
      setVerificaciones(v => v.filter(item => item.id !== id));
      
      // Actualizar contador local
      setUsuarios(prev => prev.map(u => {
        const matchingVerif = verificaciones.find(x => x.id === id);
        ...
        footer={
          <>
            ...
            <button 
              ...
              disabled={submittingRejection}
            >
              {submittingRejection ? 'Rechazando...' : 'Confirmar Rechazo'}
            </button>
          </>
        }
      >
