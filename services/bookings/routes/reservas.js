...
// ============ EMAIL CONFIG (Nodemailer SMTP + Fallback OAuth2) ============
...
const sendEmail = async ({ to, subject, html }) => {
  // 1. Intentar primero con Nodemailer SMTP
  // 1. Gmail REST API vía OAuth2 (HTTP — funciona en Render)
  if (process.env.GMAIL_CLIENT_ID && process.env.GMAIL_CLIENT_SECRET && process.env.GMAIL_REFRESH_TOKEN) {
    try {
      ...
      console.log(`✅ [Bookings OAuth2] Notificación enviada a ${to}. Id: ${result.id}`);
      return result;
    } catch (err) {
      console.warn(`⚠️ [Bookings OAuth2 Error]: ${err.message}`);
    }
  }

  // 2. Nodemailer SMTP fallback (funciona local, puede colgarse en Render)
  try {
    ...
    console.log(`✅ [Bookings SMTP] Email enviado a ${to}. Id: ${info.messageId}`);
    return info;
  } catch (smtpErr) {
    console.error(`❌ [Bookings SMTP Error]: ${smtpErr.message}`);
  }
};
